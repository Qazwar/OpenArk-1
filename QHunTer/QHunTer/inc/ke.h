...
typedef struct _KPROCESS {
    ...
    LIST_ENTRY ProfileListHead;//性能分析节点（如果参与的话）

    //
    // The following fields are referenced during context switches.
    //

    ULONG_PTR DirectoryTableBase[2];//指向页目录表地址和该进程的超空间页目录地址

#if defined(_X86_)

    KGDTENTRY LdtDescriptor;
    KIDTENTRY Int21Descriptor;
    USHORT IopmOffset;//io privilege map 
    UCHAR Iopl;
    BOOLEAN Unused;
    ...
    volatile KAFFINITY ActiveProcessors;//当前进程正在那些处理器上运行
    ...
    PVOID VdmTrapcHandler;//虚拟dos机的ctrl+c处理函数
...
#define KPROCESS_DISABLE_QUANTUM_BIT 2
    ...
    union {
        struct {
            LONG AutoAlignment : 1;//内存对齐检查
            LONG DisableBoost : 1; //提升
            LONG DisableQuantum : 1;//时限
