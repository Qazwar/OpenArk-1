...
BOOLEAN ArkHideProcess(PCHAR pIndata, ULONG cbInData, StuProcInfo * pOutData, ULONG cbOutData)
{
	...
	__try
	{
		...
		LPVOID procId = (LPVOID)GETQWORD(pIndata);
		PEPROCESS process;
		NTSTATUS st;
...
BOOLEAN PsGetProcessPathByPeb(PEPROCESS process, LPVOID procId, PWSTR path, int *pathLen)
{
...
void  FillProcessInfo(ULONG_PTR process, StuProcInfo *pOutData, ULONG cbOutData)
{
	...
	*/
	...
	if (result)
	{
		...
		result = SIZEOFPROCINFO * nProcessNum;
		if (result < cbOutData)
		{
			...
			if (pStuProcInfo[nProcessNum].ProcessId != 0 && pStuProcInfo[nProcessNum].ProcessId != 4) {
				result = PsGetProcessPath((PEPROCESS)process, (LPVOID)pStuProcInfo[nProcessNum].ProcessId, &pwStrProcPath, 0);// 接受一个进程路径的缓冲区地址
				if (result)
				{
...
BOOLEAN  PsGetProcessPath(PEPROCESS process, LPVOID procId, OUT PWSTR *pStrProcPath, ULONG *pathLen)
{
	using namespace NT::EPROCESS;
	...
	if (process && process > MmSystemRangeStart) {
		...
		sectionObject = (PSECTION_OBJECT)GETQWORD((ULONG_PTR)process + SectionObjectOffset);// 得到section_object
		if (sectionObject) {

			/*if (NT::OsVersion >= Windows10_2015)
				v15 = sub_FFFFF8800646F630(pSectionObject, ppBuf_1, pReturnLen_1);*/
